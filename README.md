...
+ I copied the captured data into libre office calc
+ used the build functions to determine the standard deviation
+ put the values in the config/6_Sensornoise.txt

## Criteria 2
_Implement a better rate gyro attitude integration scheme in the UpdateFromIMU() function._
+ use the FromEuler123_RPY function in Quaternion class, for creating a quaternion from Euler Roll/PitchYaw
+ use IntegrateBodyRate function
+ use quaternion.Pitch(), quaternion.Roll() and quaternion.Yaw();
+ normalize ekfState (Yaw)

## Criteria 3
_Implement all of the elements of the prediction step for the estimator._
+ Predict
  + use the current accelerations and body rates for prediction
  + update the ekfCov according to the EKF equation
+ PredictState
  + calculate x, y, z
  + convert body accelerations to global accelerations
  + calculate x_dot, y_dot, z_dot
+ GetRbgPrime
  + use sin() and cos() functions in the right place
  + use pitch, roll and yaw for compute RbgPrime


## Criteria 4
_Implement the magnetometer update._
+ use current estimated yaw from ekfState(6)
+ calculate difference between measured and estimated yaw
+ normalize the difference

## Criteria 5
_Implement the GPS update._
+ use hPrime and estimate from ekfState


## Criteria 6
_Meet the performance criteria of each step._

+ 6: Pass
+ 7: Pass
+ 8: N/A
+ 9: N/A
+ 10: Pass but crash at the end sometimes
+ 11: Pass but touches ground


## Criteria 7
_De-tune your controller to successfully fly the final desired box trajectory with your estimator and realistic sensors._

+ I de-tuned my controller so it passes the passes the scenario, but it touches the ground.
The might be not the best result...
...
## original README estimation Project 
